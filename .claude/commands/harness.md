...
탐색 전 아래 파일들에 채워지지 않은 placeholder가 있는지 확인한다.
...
각 파일을 읽고 `{` 문자가 포함된 줄을 찾는다.

- **placeholder 발견 시**: 어떤 파일의 어떤 항목이 비어있는지 목록으로 제시하고, 채운 뒤 다시 `/harness`를 실행하도록 안내한 후 즉시 중단한다.
- **placeholder 없음**: 다음 단계(A)로 진행한다.
...
논의가 완료되면 설계 결정을 `docs/specs/YYYY-MM-DD-<topic>-design.md`에 저장한다:
- 해결하려는 문제
- 선택한 접근방식과 이유
- 트레이드오프
- 범위에서 제외한 것들
...
8. **품질 기준** — "TBD", "이전 step과 유사", "엣지 케이스 처리" 같은 플레이스홀더 절대 금지. 각 step은 2-5분 내 완료 가능한 단위로 분할하고, 정확한 파일 경로와 함수/클래스 시그니처를 명시한다. 완전하지 않은 코드 스니펫 금지.
