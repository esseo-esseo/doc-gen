...
# ---------------------------------------------------------------------------
# _apply_parallel_result
# ---------------------------------------------------------------------------

class TestApplyParallelResult:
    def test_completed_from_status_file(self, executor, phase_dir):
        (phase_dir / "step2-status.json").write_text(
            json.dumps({"status": "completed", "summary": "UI 구현 완료"})
        )
        executor._run_git = lambda *a: MagicMock(returncode=0, stdout="", stderr="")

        executor._apply_parallel_result({"step": 2, "name": "ui"}, elapsed=30)

        index = ex.StepExecutor._read_json(phase_dir / "index.json")
        s = next(s for s in index["steps"] if s["step"] == 2)
        assert s["status"] == "completed"
        assert s["summary"] == "UI 구현 완료"
        assert "completed_at" in s

    def test_error_when_status_file_missing(self, executor, phase_dir):
        executor._run_git = lambda *a: MagicMock(returncode=0, stdout="", stderr="")
        executor._update_top_index = MagicMock()

        executor._apply_parallel_result({"step": 2, "name": "ui"}, elapsed=10)

        index = ex.StepExecutor._read_json(phase_dir / "index.json")
        s = next(s for s in index["steps"] if s["step"] == 2)
        assert s["status"] == "error"
        assert "status.json" in s["error_message"]

    def test_blocked_status(self, executor, phase_dir):
        (phase_dir / "step2-status.json").write_text(
            json.dumps({"status": "blocked", "blocked_reason": "API 키 없음"})
        )
        executor._run_git = lambda *a: MagicMock(returncode=0, stdout="", stderr="")
        executor._update_top_index = MagicMock()

        executor._apply_parallel_result({"step": 2, "name": "ui"}, elapsed=5)

        index = ex.StepExecutor._read_json(phase_dir / "index.json")
        s = next(s for s in index["steps"] if s["step"] == 2)
        assert s["status"] == "blocked"
        assert s["blocked_reason"] == "API 키 없음"
        assert "blocked_at" in s

    def test_error_status_from_file(self, executor, phase_dir):
        (phase_dir / "step2-status.json").write_text(
            json.dumps({"status": "error", "error_message": "컴파일 실패"})
        )
        executor._run_git = lambda *a: MagicMock(returncode=0, stdout="", stderr="")
        executor._update_top_index = MagicMock()

        executor._apply_parallel_result({"step": 2, "name": "ui"}, elapsed=15)

        index = ex.StepExecutor._read_json(phase_dir / "index.json")
        s = next(s for s in index["steps"] if s["step"] == 2)
        assert s["status"] == "error"
        assert s["error_message"] == "컴파일 실패"

    def test_malformed_status_file_treated_as_error(self, executor, phase_dir):
        (phase_dir / "step2-status.json").write_text("not-json{{{")
        executor._run_git = lambda *a: MagicMock(returncode=0, stdout="", stderr="")
        executor._update_top_index = MagicMock()
...
class TestCreatePr:
    def test_calls_gh_pr_create(self, executor):
        mock_result = MagicMock(returncode=0, stdout="https://github.com/pr/1", stderr="")
        with patch("subprocess.run", return_value=mock_result) as mock_run:
            executor._create_pr()
    ...
    def test_pr_title_includes_phase_name(self, executor):
        mock_result = MagicMock(returncode=0, stdout="https://github.com/pr/1", stderr="")
        with patch("subprocess.run", return_value=mock_result) as mock_run:
            executor._create_pr()
        ...
        assert "mvp" in cmd[title_idx]

    def test_pr_body_includes_step_summaries(self, executor):
        mock_result = MagicMock(returncode=0, stdout="https://github.com/pr/1", stderr="")
        with patch("subprocess.run", return_value=mock_result) as mock_run:
            executor._create_pr()
        ...
        assert "setup" in cmd[body_idx]
        assert "core" in cmd[body_idx]

    def test_handles_gh_failure_gracefully(self, executor, capsys):
        mock_result = MagicMock(returncode=1, stdout="", stderr="gh: command not found")
        with patch("subprocess.run", return_value=mock_result):
            executor._create_pr()  # should not raise

        out = capsys.readouterr().out
        assert "WARN" in out

    def test_hints_on_gh_not_found(self, executor, capsys):
...
# ---------------------------------------------------------------------------
# main() — --pr, --parallel 플래그
# ---------------------------------------------------------------------------

class TestMainCliNewFlags:
    def test_parallel_flag_sets_option(self, tmp_project, phase_dir):
        with patch("sys.argv", ["execute.py", "0-mvp", "--parallel"]):
            with patch.object(ex, "ROOT", tmp_project):
                with patch.object(ex.StepExecutor, "run"):
                    executor = ex.StepExecutor.__new__(ex.StepExecutor)
                    with patch.object(ex, "ROOT", tmp_project):
                        inst = ex.StepExecutor("0-mvp", parallel=True)
                    assert inst._parallel is True

    def test_pr_flag_sets_option(self, tmp_project, phase_dir):
        with patch.object(ex, "ROOT", tmp_project):
            inst = ex.StepExecutor("0-mvp", auto_pr=True)
        assert inst._auto_pr is True

    def test_pr_implies_push(self, tmp_project, phase_dir):
        with patch.object(ex, "ROOT", tmp_project):
            inst = ex.StepExecutor("0-mvp", auto_pr=True)
        # --pr 단독으로도 push가 실행되어야 함 (auto_push OR auto_pr)
        assert inst._auto_pr is True
